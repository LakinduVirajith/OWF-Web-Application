...
export const getVocationalTrainingDetails = async (req, res) => {
  try {
    ...
    const vocationalTraining = await fetchVocationalTrainingDetailsById(
      courseId
    );
    if (!vocationalTraining)
      return res.status(404).json({ error: 'Course not found' });
  ...
  try {
    ...
    if (!courseName || !courseType || !formData) {
      return res
        .status(400)
        .json({
          error: 'Missing required fields: courseName, courseType, or formData',
        });
    }

    const result = await addApplicationDetails(
      courseName,
      courseType,
      formData
    );
    ...
    if (result.success) {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to submit application', details: result.error });
    }
  } catch (err) {
