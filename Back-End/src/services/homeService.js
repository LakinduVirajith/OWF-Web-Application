...
export const fetchHomeCarousel = async () => {
    ...
    return imageFiles.map((file) => {
        ...
        const [name = "", description = ""] = baseName.split(" - ").map(part => part.trim());
...
const fetchCourseWithImage = async (folderName) => {
    const { excelFile, imageFiles } = await getChildFolderFiles("Courses", folderName);
    ...
    const isSingleRowCourse = ["Pre-School", "Primary-Education", "Sports", "Music"].includes(folderName);
...
export const saveContactMessage = async (formData) => {
    ...
    const existingIds = sheet.map(row => row.ID).filter(id => /^CU_\d+$/.test(id));
    const maxIdNum = Math.max(0, ...existingIds.map(id => parseInt(id.split('_')[1], 10)));
...
export const fetchHomeEvents = async () => {
    ...
    return sheet
        .filter(event => String(event.Visibility).toLowerCase() === 'true' && String(event.H_Visibility).toLowerCase() === 'true')
        .map(event => ({
        id: event.ID,
        title: event.Header,
        date: excelDateToJSDate(event.Date) || event.Start,
        time: event.Time,
