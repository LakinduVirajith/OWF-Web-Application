...
export const fetchFolderByName = async (folderName) => {
  const res = await graphClient.api(`/users/${process.env.ONE_DRIVE_USER_ID}/drive/root:/${process.env.ONE_DRIVE_FOLDER_NAME}:/children`).get();
  const folder = res.value.find(item => item.name === folderName && item.folder);
  return folder || null;
};

export const fetchSubfolders = async (folderId) => {
  const res = await graphClient.api(`/users/${process.env.ONE_DRIVE_USER_ID}/drive/items/${folderId}/children`).get();
  return res.value
...
export const fetchFoldersWithImages = async (folderId, page = 1, limit = 2) => {
  const rootRes = await graphClient
    .api(`/users/${process.env.ONE_DRIVE_USER_ID}/drive/items/${folderId}/children`)
    .get();
  ...
  const results = await Promise.all(
    pagedFolders.map(async folder => {
      const childrenRes = await graphClient
        .api(`/users/${process.env.ONE_DRIVE_USER_ID}/drive/items/${folder.id}/children`)
        .get();
